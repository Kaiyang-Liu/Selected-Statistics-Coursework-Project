...
temp1<- data.frame(test_combats %>% dplyr::select(winner_first_label,Diff_attack ,Diff_defense, Diff_sp_defense,Diff_sp_attack,Diff_speed ,Diff_HP, First_pokemon_legendary, Second_pokemon_legendary,type_relationship))


#temp1$winner_first_label = ifelse(temp1$winner_first_label <0,FALSE,TRUE)
#temp1$winner_first_label = as.numeric(temp1$winner_first_label)

temp1$winner_first_label = ifelse(temp1$winner_first_label == 'no', 0, 1)
temp1$First_pokemon_legendary = ifelse(temp1$First_pokemon_legendary == 'False', 0, 1)
temp1$Second_pokemon_legendary = ifelse(temp1$Second_pokemon_legendary == 'False', 0, 1)

temp2 = temp1

temp2$winner_first_label = as.factor(temp2$winner_first_label)
temp2$First_pokemon_legendary = as.factor(temp2$First_pokemon_legendary)
temp2$Second_pokemon_legendary = as.factor(temp2$Second_pokemon_legendary)

set.seed(1234)
split <- createDataPartition(y=temp1$winner_first_label, p = 0.75, list = FALSE)
train <- temp1[split,]
test <- temp1[-split,]
...
sum(diag(table(prrr, test$winner_first_label)))/nrow(test)

#Make the figure of neural network
library(devtools)
